...
Matriz* inicializaMatriz (int nlinhas, int ncolunas){
    ...
    m->matriz = (int *) malloc(nlinhas * ncolunas * sizeof(int*));
...
void modificaElemento (Matriz* mat, int linha, int coluna, int elem){
    mat->matriz[(linha * mat->linhas) + coluna] = elem;
}
...
int recuperaElemento(Matriz* mat, int linha, int coluna){
    return mat->matriz[(mat->linhas * linha) + coluna];
}
...
Matriz* transposta (Matriz* mat){
    ...
    for (int i = 0; i < mat->linhas; i++){
        for (int j = 0; j < mat->colunas; j++){
            m->matriz[(j * mat->colunas) + i] = mat->matriz[(mat->linhas * i) + j];
        }
    }
    return m;
}
    ...
    for (int i = 0; i < mat1->linhas; i++){
        for (int h = 0; h < mat2->colunas; h++){
            ...
            for (int j = 0; j < mat1->colunas; j++){
                temp += mat1->matriz[(mat1->linhas * i) + j] * mat2->matriz[(mat2->linhas * j) + h];
            }
            m->matriz[(m->linhas * i) + h] = temp;
        }
    }
    return m;
}
...
void imprimeMatriz(Matriz* mat){
    for (int i = 0; i < mat->linhas; i++){
        for (int j = 0; j < mat->colunas; j++){
            printf("%d ", mat->matriz[(mat->linhas *i) + j]);
        }
        printf("\n");
    }
...
void imprimeLinha (Matriz* mat, int indice){
    if (indice <= mat->linhas){
        for (int i = 0; i < mat->colunas; i++){
            printf("%d ", mat->matriz[(mat->linhas * indice) + i]);
        }
    }
    printf("\n");
}
